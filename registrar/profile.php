...
if (!in_array(($_SESSION['role'] ?? ''), ['registrar','system_admin'], true)) {
...
if ($stmt = $conn->prepare('SELECT username, email, first_name, last_name FROM users WHERE id = ? LIMIT 1')) {
    $stmt->bind_param('i', $user_id);
    if ($stmt->execute()) {
        $stmt->bind_result($u, $e, $f, $l);
        if ($stmt->fetch()) {
            $current['username'] = $u;
            $current['email'] = $e;
            $current['first_name'] = $f;
            $current['last_name'] = $l;
        }
    }
    $stmt->close();
                        ...
                        <div class="card-body">
                            ...
                            <form method="post" action="">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="username" class="form-control" value="<?php echo htmlspecialchars($current['username']); ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Email</label>
                                    <input type="email" name="email" class="form-control" value="<?php echo htmlspecialchars($current['email']); ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">First Name</label>
                                    <input type="text" name="first_name" class="form-control" value="<?php echo htmlspecialchars($current['first_name']); ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Last Name</label>
                                    <input type="text" name="last_name" class="form-control" value="<?php echo htmlspecialchars($current['last_name']); ?>" required>
                                </div>
                                <div class="mb-3">
