...
// Handle Grade Entries CRUD (for school admin)
if ($section === 'grades') {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $mode = $_POST['mode'] ?? 'create';
        // When updating, student may be disabled; accept hidden field fallback
        $student_id = intval($_POST['student_id'] ?? ($_POST['student_id_hidden'] ?? 0));
        if ($student_id === 0) { $student_id = intval($_POST['student_id_hidden'] ?? 0); }
        ...
        // Auto-generate descriptors if enabled and remarks not provided
        ...
        if ($student_id <= 0 || $subject_id <= 0 || $grade_val === '' || $quarter === '' || $school_year === '') {
            ...
            if (!$has_quarter_col && !in_array($quarter, ['1st','2nd'], true)) {
                $error_msg = 'The database is not yet migrated to quarters. Please ask the system admin to run the SQL migration to enable 3rd/4th quarter entries.';
            } else if ($mode === 'create') {
                if ($has_quarter_col) {
...
// Detect if 'quarter' column exists (for pre-migration compatibility)
$has_quarter_col = false;
$colCheck = $conn->query("SHOW COLUMNS FROM grades LIKE 'quarter'");
if ($colCheck) { $has_quarter_col = ($colCheck->num_rows > 0); $colCheck->close(); }
                                ...
                                <?php else: foreach ($grade_rows as $g): ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a class="btn btn-sm btn-outline-primary" href="../students/report_card.php?student_id=<?php echo (int)($g['student_id'] ?? 0); ?>" target="_blank" title="Print Report Card"><i class="bi bi-printer"></i></a>
                                        </td>
                                    </tr>
                                <?php endforeach; endif; ?>
                                </tbody>
                ...
                <div class="modal fade" id="gradeModal" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog">
                        <form method="post" class="modal-content">
                            <div class="modal-header"><h5 class="modal-title">Grade Entry</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                            <div class="modal-body">
                                <input type="hidden" name="mode" id="grade_mode" value="create">
                                <input type="hidden" name="id" id="grade_id" value="0">
                                <input type="hidden" name="student_id_hidden" id="grade_student_hidden" value="">
                                <div class="mb-2">
